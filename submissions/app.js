...
function createGroup(e) {
    ...
    if (!name.trim() || groups.find(g => g.name === name)) { alert(groups.find(g => g.name === name) ? 'Group exists' : 'Name required'); return; }
    groups.push({ id: Date.now(), name, description: document.getElementById('groupDescription').value || '', createdAt: new Date().toISOString(), memberCount: 0, taskCount: 0 });
    db.setGroups(groups);
    alert('✅ Group created!');
...
function addMember(e) {
    ...
    if (!name || !email || members.find(m => m.email === email)) { alert('Invalid or exists'); return; }
    members.push({ id: Date.now(), name, email, groupId: parseInt(groupId), totalPoints: 0, tasksCompleted: 0, tasksPending: 0, joinedAt: new Date().toISOString() });
...
function createTask(e) {
    ...
    tasks.push({ id: Date.now(), title: document.getElementById('taskTitle').value, description: document.getElementById('taskDescription').value || '', deadline: document.getElementById('taskDeadline').value, priority: document.getElementById('taskPriority').value, points: parseInt(document.getElementById('taskPoints').value), status: 'PENDING', assignedMemberId: memberId, groupId, createdAt: new Date().toISOString(), completedAt: null });
    db.setTasks(tasks);
    member.tasksPending++;
    db.setMembers(members);
    const groups = db.getGroups();
...
function deleteTask(groupId, taskId) {
    ...
    db.setMembers(members);
    db.setTasks(tasks.filter(t => t.id !== taskId));
    const groups = db.getGroups();
    const g = groups.find(g => g.id === groupId);
